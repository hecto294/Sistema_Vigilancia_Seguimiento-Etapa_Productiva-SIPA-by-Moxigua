// src/pages/apoyo/MiPerfil.jsx
import React from 'react';

const MiPerfil = ({ user }) => {
  const perfil = {
    nombre: user?.nombre || 'Asistente Administrativo',
    rol: 'Apoyo Administrativo (Solo Lectura)',
    email: 'asistente@soy.sena.edu.co',
    centro: 'CGMLTI - Centro de Gestión de Mercados, Logística y Tecnologías de la Información',
    area: 'Apoyo Administrativo',
    permisos: ['Consulta de usuarios', 'Consulta de fichas', 'Visualización de reportes'],
    avance: 60
  };
      ...
      <div style={{ 
        ...
        <div style={{ textAlign: 'center', marginBottom: '20px', borderBottom: '1px solid #e5e7eb', paddingBottom: '20px' }}>
          <div style={{ 
            ...
            <img src={user?.avatar || 'https://i.pravatar.cc/150?img=9'} alt="Avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}>
          ...
          <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px' }}>
            <p style={{ fontWeight: 'bold', color: '#3ca203', fontSize: '14px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="fas fa-user-check" /> Datos del Apoyo
            </p>
          ...
          <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px' }}>
            <p style={{ fontWeight: 'bold', color: '#3ca203', fontSize: '14px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="fas fa-lock" /> Permisos del Sistema
            </p>
            {perfil.permisos.map((permiso, idx) => (
              <p key={idx} style={{ margin: '4px 0', fontSize: '13px', color: '#374151' }}>
                <i className="fas fa-check-circle" style={{ color: '#10b981', marginRight: '8px' }} />
                {permiso}
              </p>
            ))}
            <p style={{ margin: '4px 0', fontSize: '13px', color: '#ef4444', fontStyle: 'italic' }}>
              <i className="fas fa-ban" style={{ marginRight: '8px' }} />
        ...
        <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px' }}>
          ...
          <p style={{ margin: '8px 0 0 0', fontSize: '12px', color: '#6b7280', fontStyle: 'italic' }}>
            {perfil.avance >= 80 ? '📊 Excelente uso del sistema de consulta.' : 
             perfil.avance >= 50 ? '📋 Buen uso de las herramientas de consulta.' : 
             '🔍 Explora más las funcionalidades del sistema.'}
          </p>
        </div>
