  ...
  const [aprendices, setAprendices] = useState([
    {
      ...
      reporteFinal: { nombre: 'Reporte_Final_Laura_Martinez.pdf', tipo: 'application/pdf', tamaño: '2.1 MB', fechaSubida: '02/09/2025' }
    },
    {
      id: 2,
  ...
  return (
    <div style={{ width: '100%', padding: '20px 0' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        ...
        />
        <button onClick={handleSearch} style={{ background: '#3ca203', color: 'white', border: 'none', padding: '10px 18px', borderRadius: '8px', cursor: 'pointer', fontWeight: 500 }}>
          <i className="fas fa-search" /> Buscar
        </button>
        <button onClick={handleClear} style={{ background: '#e5e7eb', color: '#374151', border: 'none', padding: '10px 18px', borderRadius: '8px', cursor: 'pointer', fontWeight: 500 }}>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  {tieneReporte ? (
                    <button onClick={() => handleVerReporte(aprendiz)} style={{ background: '#e0f2fe', color: '#0ea5e9', border: 'none', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 500 }}>
                      <i className="fas fa-eye" /> Ver reporte
                    </button>
                  ) : (
                    <button onClick={() => handleClickSubir(aprendiz)} style={{ background: '#3ca203', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 500 }}>
                      <i className="fas fa-upload" /> Subir reporte
                    </button>
                  )}

                  {tieneReporte && (
                    <button onClick={() => handleClickSubir(aprendiz)} title="Reemplazar reporte" style={{ background: '#fef3c7', color: '#d97706', border: 'none', padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 500 }}>
