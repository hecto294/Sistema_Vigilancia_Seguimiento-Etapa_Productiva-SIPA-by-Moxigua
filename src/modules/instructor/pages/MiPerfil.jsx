// src/modules/instructor/pages/MiPerfil.jsx
...
const MiPerfil = () => {
  ...
  const cargarPerfil = async () => {
    try {
      ...
      // 🔥 Leer SIEMPRE del localStorage (usuario logueado actual)
      let localUser = null;
      try {
      ...
      // 🔥 El backend puede fallar, pero priorizamos el localStorage
      let data = null;
      try {
        data = await userService.getMe();
      } catch (err) {
        console.warn('Backend /me falló, usando localStorage:', err);
      }

      // 🔥 IMPORTANTE: localStorage GANA sobre el backend
      const combined = { ...data, ...localUser };
