// src/pages/instructor/MiPerfil.jsx
import React from 'react';

const MiPerfil = ({ user }) => {
  // Datos del perfil del instructor
  const perfil = {
    nombre: user?.nombre || 'Carlos Andrés López',
    rol: 'Instructor',
    email: 'carlos.lopez@soy.sena.edu.co',
    centro: 'CGMLTI - Centro de Gestión de Mercados, Logística y Tecnologías de la Información',
    area: 'Desarrollo de Software',
    fichasAsignadas: [
      { id: '2875901', programa: 'Análisis y Desarrollo de Software', nivel: 'Tecnólogo', aprendices: 28, estado: 'Activa' },
      { id: '2875902', programa: 'Gestión Empresarial', nivel: 'Tecnólogo', aprendices: 24, estado: 'Activa' },
    ],
    totalAprendices: 52,
    avance: 76 // Porcentaje de avance general
  };
    ...
    <div style={{ width: '100%', padding: '20px 0' }}>
      <div style={{ 
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}>
          ...
          <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px' }}>
            <p style={{ fontWeight: 'bold', color: '#3ca203', fontSize: '14px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="fas fa-user-tie" /> Datos del Instructor
            </p>
          ...
          <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px' }}>
            ...
            <p style={{ margin: '4px 0', fontSize: '13px', color: '#374151' }}>
              <strong>Fichas a cargo:</strong> {perfil.fichasAsignadas.length}
            </p>
            <p style={{ margin: '4px 0', fontSize: '13px', color: '#374151' }}>
              <strong>Total aprendices:</strong> {perfil.totalAprendices}
            </p>
            <p style={{ margin: '4px 0', fontSize: '13px', color: '#374151' }}>
              <strong>Avance general:</strong> {perfil.avance}%
            </p>
          </div>
        </div>
        ...
        <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px', marginBottom: '20px' }}>
          <p style={{ fontWeight: 'bold', color: '#3ca203', fontSize: '14px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <i className="fas fa-layer-group" /> Fichas a cargo
          </p>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {perfil.fichasAsignadas.map((ficha, idx) => (
        ...
        {/* === AVANCE GENERAL (BARRA DE PROGRESO) === */}
        <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '10px' }}>
          ...
          <p style={{ margin: '8px 0 0 0', fontSize: '12px', color: '#6b7280', fontStyle: 'italic' }}>
            {perfil.avance >= 80 ? '¡Excelente progreso! Sigues guiando a tus aprendices de forma excepcional.' : 
             perfil.avance >= 50 ? 'Vas por buen camino. Sigue así.' : 
             'Aún tienes camino por recorrer. ¡Ánimo!'}
          </p>
        </div>
