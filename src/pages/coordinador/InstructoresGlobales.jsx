...
const InstructoresGlobales = () => {
  ...
  const [instructores, setInstructores] = useState([
    ...
    { id: 4, nombre: 'Laura Rodríguez', email: 'laura@sena.edu.co', fichasAsignadas: 1 },
  ]);
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (evt) => {
        try {
          ...
          const nuevosInstructores = jsonData.map((row, idx) => ({
            id: instructores.length + idx + 1,
            nombre: row['Nombre'] || row['nombre'] || 'Sin nombre',
            email: row['Email'] || row['email'] || 'Sin email',
            fichasAsignadas: row['Fichas'] || row['fichas'] || 0
          }));

          setInstructores([...instructores, ...nuevosInstructores]);
          alert(`✅ Carga masiva exitosa. Se agregaron ${nuevosInstructores.length} instructores.`);
        } catch (error) {
    ...
    <div style={{ width: '100%', padding: '20px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ color: '#6b7280', margin: '5px 0 0 0' }}>Gestión centralizada y asignación de fichas.</p>
        </div>
        <button
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px' }}>
          ...
          <p style={{ color: '#6b7280' }}>No hay instructores con ese criterio de búsqueda.</p>
        </div>
      ) : (
